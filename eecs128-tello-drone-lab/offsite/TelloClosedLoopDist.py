...
###################################################################################################
    ...
    kp = 3.0 # <--------------------------------------------------------------------------------------- Fill this out
    ki = 0.05 
    kw = 0.7 # decay term to avoid integral windup
    
    period = 20 # <----------------------------------------------------------------------------------- Fill this out
    amplitude = 180 # <-------------------------------------------------------------------------------- Fill this out
    # Control stores
    integratedError = 0.0
    errorStore = 0.0
    ...
    for i in range(0,500):
        ...
        if ((ptime-startDist) % distPeriod) < distPeriod/2.0:
            control_UD = 100
            print('Up disturbance')
        else:
            control_UD = -100
            print('Down disturbance')
...
###################################################################################################
        ...
        control_LR = min(100, max(-100, int(np.clip(-100,100,control_LR))))
        control_FB = min(100, max(-100, int(np.clip(-100,100,control_FB))))
        control_UD = min(100, max(-100, int(np.clip(-100,100,control_UD))))
        control_YA = min(100, max(-100, int(np.clip(-100,100,control_YA))))
        #control_LR = int(np.clip(-100,100,control_LR))
        #control_FB = int(np.clip(-100,100,control_FB))
        #control_UD = int(np.clip(-100,100,control_UD))
        #control_YA = int(np.clip(-100,100,control_YA))
